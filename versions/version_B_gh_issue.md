...
The primary abstractions are defined through the lifecycle of a single model compilation. All of them are created through one API: `Observatory.enter_context(name, config)`.

1. **Session:** The outermost scope. Created automatically when the first `enter_context` is called (region stack is empty). Identified by `session_id`. This is the *only* boundary where lens lifecycle hooks fire (`on_session_start`, `on_session_end`).
2. **Region:** A named, nested scope inside a Session. Created by any *inner* `enter_context` call. Regions form a tree (e.g., `lowering/pass_1/`). They carry no lens hooks — they exist to label *where* in the pipeline each capture happened.
3. **Config:** Each `enter_context` can pass a `config` dict. It merges into the parent’s config on entry and pops on exit. This lets authors toggle lenses (e.g., disable expensive accuracy simulation for a fast sub-pipeline) without touching surrounding code.
4. **Record:** One captured observation. Created by `Observatory.collect(name, artifact)`. Stamped with the current `session_id` and the full `region_stack` path at the moment of capture.
5. **Archive:** The raw JSON file written at session close. Contains `sessions[]` + `records[]` with no analysis.
6. **Report:** The derived output generated offline from an Archive. HTML for humans, JSON for machines.
...
The Accuracy Lens follows the protocol to measure model accuracy at each pipeline stage, compare stages against a baseline, and surface results in both the HTML report table and CI JSON output.
...
* **Session Dashboard:** Shows critical run configuration and execution metadata.
  ...
  ![Session Dashboard](https://quic-boyuc.github.io/Executorch_Observatory_Demo/demo_material/session_dashboard.png)

* **Record Tree Explorer Panel:** Organizes captured records according to their logical pipeline regions.
  
  ![Record Tree Explorer](https://quic-boyuc.github.io/Executorch_Observatory_Demo/demo_material/records_explorer.png)

* **Layered Overlays:** Visualizes compiler metadata and accuracy gradients directly on graph nodes.
  
  ![Interactive FX Graph](https://quic-boyuc.github.io/Executorch_Observatory_Demo/demo_material/interactive_graph.png)
...
* **Cross-Backend Node Alignment:** Node-selection synchronization maintains layout context across side-by-side backend graphs.
  ...
  ![Cross-Backend Compare](https://quic-boyuc.github.io/Executorch_Observatory_Demo/demo_material/cross_backend_compare.png)
